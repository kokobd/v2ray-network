...
in
{
  networking.firewall = {
    ...
    extraCommands = ''
      ...
      ${iptablesCmd} -t mangle -A V2RAY -d ${settings.gateway.localNetworkRange} -p tcp -j RETURN
      ${iptablesCmd} -t mangle -A V2RAY -d ${settings.gateway.localNetworkRange} -p udp ! --dport 53 -j RETURN
      ...
      ${iptablesCmd} -t mangle -A V2RAY_MASK -d ${settings.gateway.localNetworkRange} -p tcp -j RETURN
      ${iptablesCmd} -t mangle -A V2RAY_MASK -d ${settings.gateway.localNetworkRange} -p udp ! --dport 53 -j RETURN
